...
final class iPhoneRawGyroStreamTests: XCTestCase {
    func test_records_raw_gyro_samples() async throws {
        guard CMMotionManager().isGyroAvailable else {
            throw XCTSkip("raw gyro unavailable on this device")
        }
        ...
        let stream = iPhoneRawGyroStream(streamId: "imu_gyro_raw", rateHz: 100)
